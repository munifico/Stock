...
#주가지수 조회  WHERE BASE_DT > DateSerial(2009,07,06) 
cs.execute("select BASE_DT, KOSPI_INDEX from TB_KOSPI_INDEX ORDER BY BASE_DT")
rows = cs.fetchall() 
...
for i in range(91,arrIndexVal.__len__() - 1) : 
    ...
    if arrDateVal[i].month >= 5 and arrDateVal[i].month <= 10 and False:
